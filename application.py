...
class StudentGradeApp(ctk.CTk):  
    ...
    def create_widgets(self):  
        ...
        ctk.CTkEntry(tab_grades, textvariable=self.search_course_var,placeholder_text="Search course").grid(row=3,column=1,padx=10,pady=5,sticky="ew")
    ...
    def add_student(self):
        if self.name.get() and self.index_number.get():
            try:
                ...
                self.refresh_data()
                messagebox.showinfo("Uspješno ste dodali studenta")
                self.name.set("")
